            ...
            "chapters": [
                {
                    ...
                    "content": [
                        {"type": "text", "text": "Python 是一种解释型、面向对象、动态类型的高级编程语言，由荷兰程序员 Guido van Rossum 于 1991 年首次发布。经过 30 多年的发展，Python 已经成为数据科学、人工智能、Web 开发、自动化运维等领域最受欢迎的语言之一。Python 的设计哲学强调代码的可读性和简洁性——'用一种方法，最好是只有一种方法来做一件事'。"},
                        {"type": "list", "title": "Python 核心特点（为什么学Python）", "items": [
                        ...
                        {"type": "code", "code": "# 你的第一个Python程序：Hello World\nprint('Hello, 商务数据分析!')\n\n# Python 代码不需要分号结尾\n# 注释用 # 开头，解释代码意图\n# 多行注释用三个引号包裹\n\"\"\"\n这是一个多行注释\n可以写多行说明文字\n\"\"\"", "lang": "python"},
                        {"type": "list", "title": "开发环境搭建步骤", "items": [
                        ...
                        {"type": "code", "code": "# 验证环境：在终端运行以下命令\n# python --version        # 查看 Python 版本\n# pip --version           # 查看 pip 版本\n# pip install pandas      # 安装数据分析必备库\n\n# 测试安装是否成功\nimport sys\nprint(f'Python 版本: {sys.version}')\nprint(f'Python 路径: {sys.executable}')", "lang": "python"}
                    ]
                },
                {
                    "title": "第2章 变量与基本数据类型",
                    ...
                    "content": [
                        {"type": "text", "text": "变量是存储数据的容器。在 Python 中，变量不需要声明类型——解释器会根据赋值自动推断。可以把变量理解为贴有标签的盒子：标签是变量名，盒子里装的是数据。Python 中一切皆对象，每个对象都有类型（type）、值（value）和身份（id）。"},
                        {"type": "list", "title": "Python 六大基本数据类型", "items": [
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "运算符是构建程序逻辑的基础。Python 提供了丰富的运算符来执行数学计算、值比较、逻辑判断等操作。理解运算符的优先级（例如乘除优先于加减）是写出正确代码的前提——不确定时可以用括号明确表达式。"},
                        {"type": "list", "title": "五大类运算符详解", "items": [
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "字符串是 Python 中最常用的数据类型，数据清洗工作的 80% 都在处理字符串。Python 提供了极其丰富的字符串处理方法。尤其对于数据分析师来说，处理来自 Excel、CSV、数据库的文本数据时，字符串操作是不可或缺的技能。"},
                        {"type": "list", "title": "字符串核心操作", "items": [
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "列表（list）是 Python 中最灵活、最常用的数据结构——用方括号 [] 定义，可以存放任意类型的数据。元组（tuple）用圆括号 () 定义，创建后不可修改（不可变）。在数据分析中，列表常用于存储一列数据、迭代处理结果、构建复杂数据结构。"},
                        {"type": "list", "title": "列表核心操作大全", "items": [
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "字典（dict）是 Python 中最重要、最强大的数据结构之一——以键值对（key-value）方式存储数据，查询速度极快（O(1)时间复杂度）。在数据分析中，字典常用于存储配置、缓存结果、构建映射关系、JSON 数据解析等。集合（set）用于去重和集合运算。"},
                        {"type": "list", "title": "字典核心操作", "items": [
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "条件判断和循环是编程的两大控制结构——条件判断让程序'做选择'，循环让程序'重复做'。在数据分析中，循环常用于遍历数据集、批量处理文件、迭代计算等场景。"},
                        {"type": "list", "title": "控制流核心技巧", "items": [
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "函数（Function）将一段可重用的代码封装为一个整体，是编程中最基本也是最重要的抽象机制。好的函数应该'只做一件事，并且做好'。模块（Module）则是将相关的函数、类、变量组织在一个 .py 文件中，便于管理和复用。"},
                        {"type": "list", "title": "函数设计要点", "items": [
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "文件操作是数据处理流程的重要环节——从外部文件读取原始数据，处理后将结果写回文件。Python 提供了简单易用的文件操作接口。在实际项目中，数据往往存储在 CSV、JSON、Excel 等文件中，掌握文件读写是数据分析的基本功。"},
                        {"type": "list", "title": "文件操作核心知识", "items": [
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "面向对象编程（OOP）是 Python 的核心编程范式之一。虽然数据分析工作不要求深入掌握 OOP，但理解基本的类与对象概念，能帮助你更好地使用 Pandas DataFrame（它是一个类）、Scikit-learn 的模型（也是类）等复杂库。"},
                        {"type": "list", "title": "OOP核心概念", "items": [
            ...
            "chapters": [
                {
                    ...
                    "content": [
                        {"type": "text", "text": "NumPy（Numerical Python）是整个 Python 数据科学生态系统的基石。Pandas、Matplotlib、Scikit-learn 等库都构建在 NumPy 之上。NumPy 的核心是 ndarray（N-dimensional array）——一种高效的同构多维数组，所有元素类型相同、内存连续存储，因此能够利用 CPU 的向量化指令实现极快的批量运算。"},
                        {"type": "list", "title": "NumPy 核心要点", "items": [
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "Pandas 是 Python 生态中最强大、最受欢迎的数据分析库——如果说 NumPy 是'强化版的数学计算'，那 Pandas 就是'强化版的 Excel'。Pandas 提供了两种核心数据结构：Series（一维，带标签的数组）和 DataFrame（二维，带行列标签的表格）。DataFrame 是数据分析中最常用的数据结构——可以理解为 Python 中的 Excel 工作表。"},
                        {"type": "list", "title": "Pandas 核心操作速查", "items": [
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "数据分析界有个共识：'数据清洗占整个分析工作量的 60%-80%'。真实世界的数据永远不是干净的——缺失值、重复值、异常值、格式不一致等问题几乎存在于每个数据集中。Pandas 提供了一套完整的工具来处理这些问题，掌握这些方法是成为合格数据分析师的必经之路。"},
                        {"type": "list", "title": "数据清洗核心方法", "items": [
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "数据分组聚合（Split-Apply-Combine）是数据分析中最常用的分析模式：先将数据按某个维度分成多个组（Split），再对每个组应用聚合函数（Apply），最后将结果合并（Combine）。这在 SQL 中是 GROUP BY，在 Excel 中是数据透视表，在 Pandas 中是 groupby——它是数据分析师的'瑞士军刀'。"},
                        {"type": "list", "title": "groupby 进阶技巧", "items": [
                        ...
                        {"type": "code", "code": "# 数据透视表（pivot_table）\n# 比 groupby 更直观的二维交叉表\npivot = df.pivot_table(\n    values='销售额',\n    index='城市',        # 行\n    columns='品类',       # 列\n    aggfunc='sum',        # 聚合方式\n    fill_value=0          # 空值填充\n)\nprint(f'数据透视表:\\n{pivot}')\n\n# 带边际汇总的透视表\npivot2 = df.pivot_table(\n    values='销售额',\n    index='城市',\n    columns='品类',\n    aggfunc='sum',\n    fill_value=0,\n    margins=True,           # 添加总计行列\n    margins_name='合计'\n)\nprint(f'\\n带合计的透视表:\\n{pivot2}')", "lang": "python"}
                    ]
                },
                {
                    "title": "第5章 数据合并与连接",
                    ...
                    "content": [
                        {"type": "text", "text": "现实中的分析数据往往分散在多个表格中——订单表、客户表、产品表各在不同的文件里。数据合并（Merge）就是将不同来源的数据按某种关联键组合在一起，就像 SQL 中的 JOIN 操作。"},
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "数据可视化是将数据转换为图形的艺术和科学。一张好的图表能在几秒内传达表格数据需要几分钟才能传递的信息。Python 中主要有两个可视化库：Matplotlib（底层、灵活）和 Seaborn（基于 Matplotlib、更美观、更简单的统计图表）。"},
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "统计分析为数据分析提供了方法论基础。描述性统计帮助理解数据的基本特征，推断性统计帮助我们基于样本对总体做出推断。对于商务数据分析来说，A/B 测试和相关性分析是最常用的统计方法。"},
                        {"type": "list", "title": "关键统计概念", "items": [
                ...
                },
                {
                    ...
                    "content": [
                        {"type": "text", "text": "在机器学习领域有一句名言：'数据和特征决定了模型的上限，而模型和算法只是在逼近这个上限。'特征工程就是将原始数据转化为更适合机器学习模型使用的特征的过程。"},
            ...
            "chapters": [
                {
                    ...
                    "content": [
                        {"type": "list", "title": "供应链核心KPI", "items": [
...
def build_chapter_exercises():
    """为每门课程的每个章节构建3-5道配套练习题"""
    ex = {
        "python": {
            "0": [  # 第1章 Python概述
                {"type":"single","q":"Python 的创始人是？","options":["Guido van Rossum","Dennis Ritchie","James Gosling","Brendan Eich"],"answer":0,"explain":"Python 由 Guido van Rossum 于 1991 年创建。Dennis Ritchie 创建了C语言，James Gosling 创建了Java，Brendan Eich 创建了JavaScript。"},
                {"type":"judge","q":"Python 是编译型语言，必须先编译再运行。","answer":False,"explain":"Python 是解释型语言，无需编译，直接运行。但Python内部会将代码编译为字节码(.pyc)。"},
                {"type":"single","q":"以下哪个不是 Python 的特点？","options":["语法简洁","跨平台","需要手动管理内存","丰富的第三方库"],"answer":2,"explain":"Python 自动管理内存（垃圾回收），不需要程序员手动分配和释放内存。"},
                {"type":"judge","q":"pip 是 Python 的包管理工具，用于安装第三方库。","answer":True,"explain":"pip install 包名 是最常用的安装方式。PyPI上已有超过40万个包。"}
            ],
            "1": [  # 第2章 变量与类型
                {"type":"single","q":"以下哪个变量名是不合法的？","options":["_name","name123","2name","name_2"],"answer":2,"explain":"变量名不能以数字开头。_name 以下划线开头是合法的（通常表示私有变量）。"},
                {"type":"single","q":"type(3.14) 的输出是什么？","options":["<class 'int'>","<class 'float'>","<class 'str'>","<class 'bool'>"],"answer":1,"explain":"带小数点的数字默认为 float 类型。3 才是 int。"},
                {"type":"judge","q":"Python 中布尔值 True 和 False 的首字母可以小写。","answer":False,"explain":"Python 的布尔值必须首字母大写：True 和 False，小写的 true/false 不是合法的布尔值。"},
                {"type":"fill","q":"使用 _____ 函数可以查看一个变量的类型。","answer":"type","explain":"type(x) 返回变量x的类型，是调试和学习时非常常用的函数。"}
            ],
            "2": [  # 第3章 运算符
                {"type":"single","q":"表达式 17 // 5 的结果是？","options":["3","3.4","2","4"],"answer":0,"explain":"// 是整除（地板除），17//5=3（舍去小数部分）。/ 才是普通除法，结果为3.4。"},
                {"type":"single","q":"以下哪个不是比较运算符？","options":["==","!=","=",">="],"answer":2,"explain":"= 是赋值运算符，不是比较运算符。判断等于用 ==（两个等号）。"},
                {"type":"fill","q":"Python 中，** 是 _____ 运算符。","answer":"幂","explain":"** 是幂运算，如 2**3 = 8，10**2 = 100。"},
                {"type":"judge","q":"10 > 5 and 3 > 8 的运算结果是 True。","answer":False,"explain":"and 要求两边都为 True。右边 3>8 是 False，所以整体结果是 False。"}
            ],
            "3": [  # 第4章 字符串
                {"type":"single","q":"以下哪个方法可以去除字符串首尾空白？","options":["replace()","strip()","split()","find()"],"answer":1,"explain":"strip()去除首尾空白（空格、制表符、换行符）。replace() 是替换，split() 是拆分，find() 是查找。"},
                {"type":"fill","q":"使用 f'Hello {_____}' 可以在字符串中嵌入变量，这种写法叫做 _____。","answer":"f-string","explain":"f-string 是 Python 3.6+ 最推荐的字符串格式化方式，比 % 和 .format() 更简洁高效。"},
                {"type":"single","q":'"apple,banana,orange".split(",") 的结果是？',"options":["['apple','banana','orange']","['apple,banana,orange']","'apple','banana','orange'","报错"],"answer":0,"explain":"split(',') 按逗号拆分为包含3个元素的列表。"}
            ],
            "4": [  # 第5章 列表与元组
                {"type":"single","q":"以下哪个方法可以向列表末尾添加元素？","options":["add()","insert()","append()","extend()"],"answer":2,"explain":"append()向末尾添加单个元素。extend()添加可迭代对象的每个元素。insert()在指定位置插入。"},
                {"type":"multi","q":"以下哪些操作会修改原列表？（多选）","options":["list.sort()","sorted(list)","list.append(5)","list.pop()"],"answer":[0,2,3],"explain":"sort()、append()、pop()都直接修改原列表。sorted()返回新列表，不修改原列表。"},
                {"type":"judge","q":"元组创建后不能修改其元素。","answer":True,"explain":"元组是不可变类型，创建后不能增删改。但如果元组内有列表等可变对象，列表内部元素可以修改。"},
                {"type":"fill","q":"Python 中使用 _____ 语法可以快速生成列表，如 [x*2 for x in range(10)]。","answer":"列表推导式","explain":"列表推导式(list comprehension)是Python最强大的特性之一，可以一行代码完成筛选+转换操作。"}
            ],
            "5": [  # 第6章 字典与集合
                {"type":"single","q":"以下哪种类型可以作为字典的键？","options":["列表","字典","字符串","集合"],"answer":2,"explain":"字典的键必须是不可变类型——字符串、数字、元组都可以。列表、字典、集合是可变的，不能作为键。"},
                {"type":"single","q":"d.get('key', 'default') 与 d['key'] 的区别是？","options":["没有区别","get方法键不存在时返回默认值而不是报错","get方法更快","get方法可以修改值"],"answer":1,"explain":"d['key']在键不存在时抛出KeyError。d.get('key', default)则返回默认值，安全得多。"},
                {"type":"judge","q":"集合中的元素可以重复。","answer":False,"explain":"集合的特点就是元素唯一、不重复。向集合添加重复元素不会报错但也不会改变集合。"}
            ],
            "6": [  # 第7章 控制流
                {"type":"single","q":"以下哪个关键字用于跳出循环？","options":["continue","break","return","exit"],"answer":1,"explain":"break立即终止整个循环。continue跳过当前迭代进入下一次。return从函数返回。"},
                {"type":"fill","q":"使用 _____ 函数可以在 for 循环中同时获取索引和元素值。","answer":"enumerate","explain":"for i, item in enumerate(list) 同时获得索引i和元素item，比手动维护计数器更优雅。"},
                {"type":"judge","q":"while True 会无限循环，除非循环体内有 break。","answer":True,"explain":"while True 条件永远为真，必须用break跳出，否则程序会一直运行。编写while循环时务必确保有退出条件。"}
            ],
            "7": [  # 第8章 函数
                {"type":"single","q":"以下关于函数的说法错误的是？","options":["函数可以有多个return","函数可以没有return语句","函数内部可以定义另一个函数","函数参数的类型必须事先声明"],"answer":3,"explain":"Python是动态类型语言，函数参数不需要声明类型（但可以用类型注解提示）。"},
                {"type":"fill","q":"使用 _____ 关键字可以创建一个匿名函数，如 lambda x: x*2。","answer":"lambda","explain":"lambda 函数是单行的匿名函数，常用于排序的key参数、filter/map等场景。"},
                {"type":"judge","q":"函数内部可以修改全局变量，直接赋值即可。","answer":False,"explain":"函数内直接赋值给全局变量会创建一个同名局部变量。要修改全局变量须先用 global 关键字声明。"}
            ],
            "8": [  # 第9章 文件操作
                {"type":"fill","q":"推荐使用 _____ 语句打开文件，因为它会自动关闭文件。","answer":"with","explain":"with open(...) as f: 即使发生异常也会自动关闭文件，避免资源泄漏。"},
                {"type":"single","q":"open('file.txt', 'w') 中的 'w' 表示什么？","options":["读取","写入（覆盖）","追加","二进制"],"answer":1,"explain":"'w'=write 写入模式（会覆盖原有内容），'r'=read读取，'a'=append追加，'b'=binary二进制。"},
                {"type":"judge","q":"写入中文到文件时，推荐使用 encoding='utf-8' 参数。","answer":True,"explain":"UTF-8是最通用的编码。Windows上推荐 utf-8-sig 可以避免Excel打开时中文乱码。"}
            ],
            "9": [  # 第10章 OOP
                {"type":"single","q":"类的方法中，哪个参数代表实例本身？","options":["this","self","that","instance"],"answer":1,"explain":"Python 使用 self 指代实例本身。self 是约定俗成的名字（不是关键字），但强烈建议用 self。"},
                {"type":"fill","q":"定义类时，_____ 方法在创建对象时自动调用，用于初始化属性。","answer":"__init__","explain":"__init__(self) 是构造方法，在创建对象时自动执行。可以设置初始属性值。"}
            ]
        },
        "data-analysis": {
        ...
        "data-collection": {
            "0": [{"type":"single","q":"以下哪种属于半结构化数据？","options":["CSV","JSON","SQLite数据库","Excel"],"answer":1,"explain":"JSON是有标签标记但结构灵活的数据格式。CSV和Excel属于结构化数据。"}],
            "1": [{"type":"multi","q":"Pandas可以读取以下哪些格式？（多选）","options":["CSV","Excel","JSON","SQL"],"answer":[0,1,2,3],"explain":"Pandas通过pd.read_csv/excel/json/sql可以读取所有这些格式，但SQL需要数据库驱动。"}],
            "2": [{"type":"judge","q":"df.query('sales > 1000') 比 df[df['sales'] > 1000] 更简洁且通常更快。","answer":True,"explain":"query() 方法语法更简洁，且内部使用了优化，对大数据集效率更高。"}],
            "3": [{"type":"single","q":"数据合并时，左连接(how='left')保留哪些行？","options":["左表所有行","右表所有行","两表共有行","所有行"],"answer":0,"explain":"左连接保留左表所有行，右表匹配不到的用NaN填充。"}],
            "4": [{"type":"multi","q":"以下哪些属于数据质量评估维度？（多选）","options":["完整性","准确性","美观性","一致性"],"answer":[0,1,3],"explain":"美观性不属于数据质量维度。完整性、准确性、一致性、唯一性、时效性、有效性是六大核心维度。"}],
            "5": [{"type":"fill","q":"使用 Python 的 _____ 库配合 BeautifulSoup 可以进行Web数据采集。","answer":"requests","explain":"requests 发送HTTP请求获取网页，BeautifulSoup解析HTML提取数据。两者配合是爬虫的标准组合。"}],
            "6": [{"type":"single","q":"导出CSV文件给Excel用户时，推荐什么编码？","options":["utf-8","gbk","utf-8-sig","ascii"],"answer":2,"explain":"utf-8-sig 在文件头加了BOM，Excel能正确识别中文。普通utf-8在Excel中可能显示乱码。"}],
            "7": [{"type":"judge","q":"数据处理流程应该先合并再清洗。","answer":False,"explain":"一般情况下应先对各表单独清洗，再合并。如果在合并后清洗，可能因为数据来源不清而难以定位问题。"}]
        },
        "supply-chain": {
            "0": [{"type":"single","q":"以下哪个指标衡量库存管理效率？","options":["毛利率","库存周转率","资产负债率","市盈率"],"answer":1,"explain":"库存周转率=销售成本/平均库存，越高说明商品流动越快，资金占用越低。"}],
            "1": [{"type":"fill","q":"ABC分类中，占销售额约70%的少量商品属于 _____ 类。","answer":"A","explain":"A类：少品种、高价值（约占70%销售额）。B类：中等。C类：多品种、低价值。"}],
            "2": [{"type":"single","q":"以下哪种运输方式的单位成本最低？","options":["公路","铁路","航空","水运"],"answer":3,"explain":"水运的吨公里成本最低，但时效最慢。航空成本最高但最快。选择合适的运输方式是物流优化的关键。"}],
            "3": [{"type":"multi","q":"关联规则分析中以下哪些是正确的指标含义？（多选）","options":["支持度=同时含A和B的交易/总交易","置信度=含A的交易中同时含B的比例","提升度>1表示正相关","提升度=支持度×置信度"],"answer":[0,1,2],"explain":"提升度=置信度/B的支持度，不是支持度×置信度。Lift>1表示推荐组合有意义。"}],
            "4": [{"type":"single","q":"RFM模型中，F代表什么？","options":["购买金额","购买频率","最近购买时间","客户年龄"],"answer":1,"explain":"R=Recency(最近购买时间)，F=Frequency(购买频率)，M=Monetary(消费金额)。"}],
            "5": [{"type":"judge","q":"移动平均的窗口越大，对近期变化的反应越灵敏。","answer":False,"explain":"窗口越大，平滑效果越强，但对近期变化的反应越迟钝。窗口小则更灵敏但噪声更大。"}],
            "6": [{"type":"single","q":"供应商评估中，哪个权重通常最高？","options":["服务","成本","质量","位置"],"answer":2,"explain":"质量通常是供应商评估中权重最高的维度，因为质量问题导致的损失（退货、停工、客户流失）远超其他方面。"}],
            "7": [{"type":"judge","q":"供应链健康度评估只需要看成本指标。","answer":False,"explain":"供应链健康度需综合评估：成本、效率(周转率)、服务水平(交付率)、质量(合格率)等多个维度。"}]
        }
    }
    return ex
...
exercises = build_chapter_exercises()

# 构建JS代码
js_code = """// ==================== 学生学习系统 - 共享数据与函数 ====================
// 版本：v2.0 扩充版（36章详细知识点 + 120+练习题）

// ==================== 课程数据 ====================
var courses = COURSES_PLACEHOLDER;

// ==================== 章节练习题数据 ====================
var chapterExercises = EXERCISES_PLACEHOLDER;
...
# 将 courses 和 exercises 转换为 JSON 嵌入
...
js_code = js_code.replace('COURSES_PLACEHOLDER', courses_json)
js_code = js_code.replace('EXERCISES_PLACEHOLDER', exercises_json)

with open('common.js', 'w', encoding='utf-8') as f:
    f.write(js_code)

print(f'common.js generated successfully!')
print(f'  Courses: {len(courses)} courses with {sum(len(c["chapters"]) for c in courses)} chapters')
total_ex = sum(len(v) for c in exercises.values() for v in c.values())
print(f'  Exercises: {total_ex} questions')
